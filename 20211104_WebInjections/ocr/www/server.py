...
def equation():
    ...
    if file:
        try:
            ...
            if (
                formated_text.count("(") > 1
                or formated_text.count(")") > 1
                or formated_text.count("[") > 1
                or formated_text.count("]") > 1
            ):
                return render_template("result.html", result="We can not solve complex equations for now !")
            if any(
                i in formated_text for i in ["import", "exec", "compile", "tesseract", "chr", "os", "write", "sleep"]
            ):
            ...
            if "==" in formated_text:
                ...
                if pa_1 == pa_2:
                    ...
                    return render_template(
                        "result.html", result="Sorry but it seems that %d is not equal to %d" % (pa_1, pa_2)
                    )
            else:
